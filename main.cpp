...
int main(int argc, char **argv) {
	int quit, frames, rc, activeOption, points, numberOfHits, numberOfgettingShooted, numberOfBonuses;
	...
	while(!quit) {
		...
		while (menu && !quit)
		{
			...
			while (options && !quit)
			{
				...
				DrawString(screen, screen->w / 2 - strlen(text) * 8 / 2, screen->h / 2, text, charset);
				/*sprintf(text, "3. Horizontal acceleration: ");
				DrawString(screen, screen->w / 2 - strlen(text) * 8 / 2, screen->h / 2 + 30, text, charset);
		...
		//		SCREEN_HEIGHT / 4 + cos(distance) * SCREEN_HEIGHT / 3);
			...
			if (worldTime - bonusesTime > timeEveryBonusSEC)	 //the 1 is the second that bonuses will change place
			{
				for (int i = 0; i < numberOfBonuses; i++)
				{
						bonuses[i]->setPos(rand() % 640, rand() % 480);
